...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="has2020">
        ...
        <xsl:param name="ln"></xsl:param>
        <xsl:param name="ns"></xsl:param>
        <xsl:for-each select="$ns">
            <xsl:if test="$n + . + $ln = 2020">
                <answer>
                    <xsl:value-of select="format-number($ln * $n * ., '0')"></xsl:value-of>
                </answer>
                <debug>
                    <a>
    ...
    <xsl:template name="has2020-2">
        <xsl:param name="n"></xsl:param>
        <xsl:param name="ns"></xsl:param>
        <xsl:for-each select="$ns">
            <xsl:call-template name="has2020">
                <xsl:with-param name="ln" select="$n"></xsl:with-param>
                <xsl:with-param name="n" select="."></xsl:with-param>
                <xsl:with-param name="ns" select="$ns"></xsl:with-param>
            </xsl:call-template>
        </xsl:for-each>
    </xsl:template>
    <xsl:template match="/">
        ...
        <xsl:for-each select="$numbers">
            <xsl:call-template name="has2020-2">
                <xsl:with-param name="n" select="."></xsl:with-param>
